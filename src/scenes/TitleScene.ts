...
import { FrameDataMap, GameTime, onSceneEndHandler } from '../engine/types.js';
...
export class TitleScene extends Scene {
	...
	music = document.querySelector<HTMLAudioElement>('audio#title');
	image = document.querySelector<HTMLImageElement>('img#title');

	frames: FrameDataMap = new Map([
  ...
  selectedDifficulty = 1; // Default to NORMAL
	...
	constructor(_, onEnd: onSceneEndHandler) {
		super();
	...
	private updateLogo(time: GameTime) {
		if (this.currentState !== TitleState.LOGO || time.previous < this.logoEffectTimer) return;

		this.logoEffectPosition += 1;
		this.logoEffectTimer = time.previous + FRAME_TIME * 2;
		if (this.logoEffectPosition > 71) this.currentState = TitleState.READY;
	}

	private updateClouds(time: GameTime) {
		for (const cloud of this.clouds) {
			cloud.x += cloud.velocity * time.secondsPassed;
		}
	}

	private updateScenery(time: GameTime) {
		if (this.currentState !== TitleState.SCENERY) return;

		this.scenery.y += this.scenery.velocity * time.secondsPassed;
		if (this.scenery.y > 152) this.currentState = TitleState.LOGO;
	}

	private updateOptions(time: GameTime) {
		if (this.currentState !== TitleState.READY) return;

		this.optionsAlpha = Math.min(1, this.optionsAlpha + 2 * time.secondsPassed);
	}

	private updateControls() {
    if (!control.isControlPressed(0, Control.START)) return;
    ...
    if (!this.fade.hasCompleted()) return;
  ...
  private updateDifficultySelection() {
    if (this.currentState !== TitleState.DIFFICULTY) return;
  ...
  private drawDifficultyOptions(context: CanvasRenderingContext2D) {
    if (this.currentState !== TitleState.DIFFICULTY) return;
	...
	private drawSkyBackground(context: CanvasRenderingContext2D) {
		context.fillStyle = 'rgb(85 115 175)';
		context.fillRect(0, 0, context.canvas.width, context.canvas.height);
	}

	private drawClouds(context: CanvasRenderingContext2D) {
	...
	private drawLogo(context: CanvasRenderingContext2D) {
		context.drawImage(
			this.image,
			8, 79 - this.logoEffectPosition, 227, this.logoEffectPosition,
			15, 15 + 71 - this.logoEffectPosition, 227, this.logoEffectPosition,
		);

		if (this.logoEffectPosition > 71) return;

		context.drawImage(
			this.image,
			8, 79 - this.logoEffectPosition, 227, 1,
			15, 0, 227, 88 - this.logoEffectPosition,
		);
	}

	private drawOptions(context: CanvasRenderingContext2D) {
		if (this.currentState !== TitleState.READY) return;
