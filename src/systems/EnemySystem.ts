import { EnemiesPayload, GameEventType } from '../constants/events.js';
import { CollisionTile, levelData } from '../constants/levelData.js';
...
export class EnemySystem extends System<Enemy>{
	soundExitOpen = document.querySelector<HTMLAudioElement>('audio#exit-open');
	enemyCount: number;
  enemySpeed: number;

	constructor(collisionMap: ValueOf<typeof CollisionTile>[][], time: GameTime, enemyCount: number, enemySpeed: number) {
    super(collisionMap);
	...
	addInitialEnemies(levelId: number, time: GameTime) {
  ...
  add = (payload: EnemiesPayload) => {
    for (const enemy of payload.enemies) {
      this.entries.push(
        new enemy.Class(
          { x: enemy.position.column, y: enemy.position.row }, this.collisionMap, payload.time, this.remove, this.enemySpeed
        ),
      );
    }
  };
