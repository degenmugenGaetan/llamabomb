import { FrameData, FrameDataMap, FrameDimensions } from './types.js';

export function createContext(width: number, height: number, parentSelector = 'body'): CanvasRenderingContext2D {
...
const alphaFrames: FrameDataMap = new Map([
	['alpha->', [[0, 40, 8, 8], [0, 0]]],
	['alpha-0', [[8, 0, 8, 8], [0, 0]]],
...
const fontImage = document.querySelector<HTMLImageElement>('img#font');
...
export function drawTile(
	...
	context.drawImage(
		image,
		(tile % noTilesWidth) * tileSize, Math.floor(tile / noTilesWidth) * tileSize, tileSize, tileSize,
